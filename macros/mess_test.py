...
search = {"label": "???", "keys": [key_control, "e"]}

# Status'
DND = {"label": "DND", "keys": [key_control, "e", key_slash, "dnd", key_enter]}
Availabile = {"label": "Avail", "keys": [key_control, "e", key_slash, "available", key_enter]}
...
app = {                       # REQUIRED dict, must be named 'app'
    'name' : 'Messaging', # Application name
    'macros' : [              # List of button macros...
        # COLOR    LABEL    KEY SEQUENCE
        # 1st row ----------
        (green, search["label"], search["keys"]),
        (teal, find["label"], find["keys"]),
        (red,pgup["label"], pgup["keys"]),
        # 2nd row ----------
        (white, DND["label"], DND["keys"]),
        (black, "", [key_command, "w"]),  # Close window/tab
        (red,pgdn["label"], pgdn["keys"]),

        # 3rd row ----------
        (blue, available["label"],available["keys"]),
        (black, "", [key_command, "w"]),  # Close window/tab
        (black, "", [key_command, "w"]),  # Close window/tab
        # 4th row ----------
        (black, "", [key_command, "w"]),  # Close window/tab
